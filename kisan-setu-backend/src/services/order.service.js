...
export async function create(buyerId,{produceId,quantity},m){const q=Number(quantity);const order=await prisma.$transaction(async tx=>{const l=await tx.produceListing.findFirst({where:{id:produceId,status:{in:['LISTED','RESERVED']},deletedAt:null}});if(!l)throw errors.notFound('Produce listing not available');const r=await tx.produceListing.updateMany({where:{id:produceId,availableQuantity:{gte:q},deletedAt:null},data:{availableQuantity:{decrement:q},reservedQuantity:{increment:q},status:'RESERVED'}});if(r.count!==1)throw errors.conflict('Quantity changed; retry');const total=q*Number(l.pricePerUnit);const o=await tx.order.create({data:{orderNumber:`KS-${Date.now()}-${Math.floor(Math.random()*10000)}`,buyerId,farmerId:l.farmerId,status:'PENDING_FARMER',subtotal:total,totalAmount:total,items:{create:{produceId,quantity:q,unitPrice:l.pricePerUnit,lineTotal:total}}},include:{items:true}});await tx.auditLog.create({data:{userId:buyerId,role:'BUYER',action:'ORDER_CREATED',entity:'Order',entityId:o.id,requestId:m.requestId,ipAddress:m.ip,metadata:{quantity:q}}});return o});await notifyUser({userId:order.farmerId,title:'New purchase request',message:`A buyer requested ${order.items[0].quantity} units.`,metadata:{orderId:order.id}});return order}
