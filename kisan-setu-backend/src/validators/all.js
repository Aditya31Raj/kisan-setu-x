...
export const produceSchema=z.object({cropId:z.string().uuid(),title:z.string().min(2).max(150),description:z.string().max(1000).optional(),location:z.string().min(2).max(150),unit:z.string().max(10).default('KG'),availableQuantity:z.coerce.number().positive(),pricePerUnit:z.coerce.number().positive()}); export const produceUpdateSchema=produceSchema.partial(); export const produceQuerySchema=z.object({q:z.string().max(100).optional(),search:z.string().max(100).optional(),farmerId:z.string().uuid().optional(),cropId:z.string().uuid().optional(),location:z.string().max(100).optional(),minPrice:z.coerce.number().nonnegative().optional(),maxPrice:z.coerce.number().nonnegative().optional(),availableOnly:z.enum(['true','false']).optional(),includeSold:z.enum(['true','false']).optional(),page:z.coerce.number().int().positive().optional(),limit:z.coerce.number().int().positive().max(100).optional()});
...
export const paymentSchema=z.object({orderId:z.string().uuid(),idempotencyKey:z.string().min(8).max(200),amount:z.coerce.number().positive().optional()}); export const paymentVerifySchema=z.object({providerPaymentId:z.string().min(3),success:z.boolean(),utr:z.string().optional()});
